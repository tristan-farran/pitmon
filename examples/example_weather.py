...
def run_weather_example():
    ...
    if not monitor.alarm_triggered:
        if monitor.baseline_locked:
            ...
            print(f"  Baseline established: {len(monitor.baseline_pits)} days")
            print(f"  Monitoring period: {len(monitor.monitoring_pits)} days")
            print(f"  Final two-sample KS: {monitor.get_state()['ks_distance']:.4f}")
        else:
            print(f"✓ Baseline collection completed, no alarm")
    ...
    for day, (forecast, obs) in enumerate(zip(forecasts, observations), 1):
        ...
        if alarm:
            ...
            print(f"   Note: Baseline (days 1-50) was well-calibrated")
            print(f"         Change detected in monitoring phase (days 51+)")
            break
    ...
    print("Key Takeaways:")
    print(
        "- PIT monitor detected miscalibration ~{} days after changepoint".format(
            monitor.alarm_time - 180 if monitor.alarm_triggered else "N/A"
        )
    )
    print(
        "- Changepoint localization estimated day {} (true: day 180)".format(
