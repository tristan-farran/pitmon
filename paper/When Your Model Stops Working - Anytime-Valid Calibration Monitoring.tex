...
\begin{abstract}
\noindent Deployed machine learning models often experience calibration drift as the world changes. To address this challenge, we present PITMonitor, a sequential method for detecting calibration changes in probabilistic models. Unlike traditional calibration tests that require fixed evaluation windows, PITMonitor provides \emph{anytime-valid} false alarm control: the probability of ever raising a spurious alarm is bounded by $\alpha$, regardless of when monitoring stops. We prove Type I error control via Ville's inequality and demonstrate detection power on three scenarios from \texttt{river}'s FriedmanDrift benchmark, comparing performance against the seven included drift detectors. Code is available at \url{https://github.com/tristan-farran/pitmon}.
\end{abstract}
...
delay by accepting a larger $\alpha$: setting $\alpha = 0.10$ lowers the threshold
to 10, requiring less accumulated evidence. Reducing $B$ (fewer histogram bins) also
increases power per observation when the direction of shift is consistent, at the cost
of less flexibility to detect diffuse concentration patterns.
