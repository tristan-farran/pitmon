...
All experiment hyperparameters live here so that all files share one
source of truth.
...
class Config:
    ...
    # Positions are jointly determined with .positions_for()
    ...
    def positions_for(self, drift_type: str) -> Tuple:
        ...
        if drift_type == "gra":  # expects exactly 2 positions
            ...
            step = self.n_post // 3  # space expansions evenly across post-drift window
            return (dp, dp + step, dp + 2 * step)
        else:
