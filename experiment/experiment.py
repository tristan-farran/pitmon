...
def run_single_trial(
    ...
    """Run one Monte-Carlo trial for all detectors.
    ...
    -------
    dict
        ``{detector_name: result_dict, ...}`` for all detectors.
    """
    ...
    # ── PITMonitor (run separately to extract changepoint) ────────────
    ...
    for det in baseline_detectors:
        ...
        trial_results[det.name] = asdict(det.result)
...
# ─── Process-pool worker helpers ─────────────────────────────────────
# The bundle is large (PyTorch model + sklearn objects).  Passing it as an
# argument would re-pickle it on every submitted task.  Instead we use the
# ProcessPoolExecutor initializer to load it once per worker process, storing
# it in a module-level dict that _run_trial_task then reads.

_WORKER: dict = {}


def _init_worker(bundle: ModelBundle, binary_threshold: float) -> None:
    """Called once per worker process; stores shared state globally."""
    _WORKER["bundle"] = bundle
    _WORKER["binary_threshold"] = binary_threshold


def _run_trial_task(args: tuple) -> dict:
    """Top-level wrapper required by ProcessPoolExecutor (must be picklable)."""
    cfg, drift_type, tw, trial_idx = args
...
def run_experiment(
    ...
    """Run the complete experiment: all scenarios, trials and detectors.
    ...
    3. Within each trial, evaluates PITMonitor and all baseline detectors using
    ...
    scenario_filter : set of str or None
        If given, only scenarios whose key (``"{drift_type}_tw{tw}"``) is in
        this set are computed.  All others are skipped.  When None, every
        scenario in ``cfg.drift_scenarios`` is run.
    ...
    for drift_type, tw in cfg.drift_scenarios:
        ...
        task_args = [
            (cfg, drift_type, tw, trial_idx) for trial_idx in range(cfg.n_trials)
        ]
        with ProcessPoolExecutor(
            max_workers=cfg.max_workers,
            initializer=_init_worker,
            initargs=(bundle, binary_threshold),
        ) as pool:
            futures = {pool.submit(_run_trial_task, a): a for a in task_args}
        ...
        # ── Aggregate ─────────────────────────────────────────────────
        summary = aggregate_results(raw_trial_results, cfg.n_stable)
