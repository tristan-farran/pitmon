...
Therefore $\sup_{t \ge 1} M_t \le \sup_{t \ge 1} \widetilde{M}_t$. Applying Ville's inequality,
\begin{equation}
...
After an alarm at time $T$, we estimate the changepoint location by selecting the split that best explains the post-split p-values as non-uniform. For each candidate $s \in \{1, \ldots, T-1\}$, we evaluate the segment $(p_{s+1}, \ldots, p_T)$ under two hypotheses, following those in Section~\ref{pits}:
\begin{itemize}
 \item $\mathbf{H_0^{(s)}}$: $p_t \sim \mathrm{Unif}(0,1)$, 
 i.e.\ $\theta = (1/B, \ldots, 1/B)$
 \item $\mathbf{H_1^{(s)}}$: bin probabilities follow a 
 $\mathrm{Dir}(\kappa, \ldots, \kappa)$ prior
\end{itemize}

A segment beginning at a true changepoint should contain p-values that are systematically non-uniform, so $H_1$ should explain it better than $H_0$.

The log Bayes factor $\mathrm{BF}(s) = p(\mathbf{n}^{(s)} \mid H_1^{(s)}) / p(\mathbf{n}^{(s)} \mid H_0^{(s)})$ quantifies the evidence for non-uniformity in the segment, with $\mathbf{n}^{(s)}$ counting the number of p-values in each bin. We then identify the changepoint as the split with the strongest evidence of non-uniformity, $\hat{\tau} = \argmax_s \log\mathrm{BF}(s)$. This procedure is consistent under mild conditions, since the Bayes factor grows exponentially in the number of post-change observations when the segment contains a true change, but grows only polynomially when it does not.

encompassing all non-unfiorm (and uniform)

compute a split-specific histogram $\mathbf{c}^{(s)} = (c_1^{(s)}, \ldots, c_B^{(s)})$, $N^{(s)} = \sum c_b^{(s)}$
...
This interpretation is supported by additional multi-step localization checks reported in Appendix~\ref{app:additional_checks}. Though LEA violates our assumption of one changepoint, with several changepoints it picks the strongest one.
