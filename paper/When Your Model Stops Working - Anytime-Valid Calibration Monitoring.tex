...
\begin{itemize}
    ...
    scenario.
    \item \textbf{GSG} (Global Slow Gradual, $\Delta t = 100$): The change spreads
    linearly across all features over a 100-sample transition window, representing
...
pre-drift samples for model training, $n_\text{stable} = 5{,}000$ pre-drift monitoring
samples that define the null-hypothesis window for FPR estimation, and $n_\text{post}
= 5{,}000$ post-drift samples for TPR estimation. The drift onset occurs at the
