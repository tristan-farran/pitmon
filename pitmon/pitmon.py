    ...
    **Thread Safety:** This class is not thread-safe. External synchronization
    ...
    def changepoint(self):
        ...
        - H1: unknown categorical probabilities with symmetric Dirichlet prior
              (Jeffreys prior: kappa = 1/2 per bin)

        The selected changepoint maximizes the log Bayes factor
        log p(data_after | H1) - log p(data_after | H0).

        The histogram and lgamma sum are maintained incrementally as the split
        moves forward, giving O(T) complexity instead of O(T²).

        Returns
        -------
        int or None
            Estimated changepoint (1-indexed in the monitoring stream), or
            None if no alarm yet.
        """
        ...
        # Precompute bin index for every p-value (O(T), avoids recomputing in loop)
        bin_indices = np.minimum((pvals * B).astype(int), B - 1)

        # Build initial histogram for k=1: counts over pvals[1:]
        counts = np.zeros(B, dtype=int)
        for b in bin_indices[1:]:
            counts[b] += 1
        N = n - 1

        # Initial lgamma sum: sum_b lgamma(counts[b] + kappa)
        lgamma_sum = sum(math.lgamma(int(c) + kappa) for c in counts)
        ...
        for k in range(1, max_k):
            ...
            # Log Bayes factor for split at k (after = pvals[k:], N elements)
            log_p_h1 = log_gamma_kappa0 - math.lgamma(N + kappa0) + lgamma_sum - B * log_gamma_kappa
            score = log_p_h1 + N * log_B  # = log_p_h1 - log_p_h0, log_p_h0 = -N*log_B

            if score > best_score:
                best_score = score
                best_cp = k + 1

            # Incrementally remove pvals[k] to prepare counts for k+1
            b = int(bin_indices[k])
            lgamma_sum -= math.lgamma(counts[b] + kappa)
            counts[b] -= 1
            lgamma_sum += math.lgamma(counts[b] + kappa)
            N -= 1
    ...
    def save(self, filepath):
        ...
        **Pickle Security Warning:** pickle can execute arbitrary code from
        ...
        if not self._uses_default_weight_schedule:
            warnings.warn(
                "This monitor uses a custom weight_schedule which cannot be serialized. "
                "The loaded monitor will use the default schedule and may behave differently.",
                UserWarning,
                stacklevel=2,
            )
