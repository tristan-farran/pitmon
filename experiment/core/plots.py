...
def plot_detection_rates(results: dict, save_dir: Path) -> plt.Figure:
    ...
    for col, scen in enumerate(scenarios):
        ...
        for m in ordered:
            ...
            fprs_disp.append(min(s.get("fpr", 0.0), FPR_CAP))
        ...
        for i, (m, tpr) in enumerate(zip(ordered, tprs)):
            c = _c(m)
            ax.plot(
                [0, tpr], [i, i], color=c, lw=0.9, alpha=0.38, solid_capstyle="round"
            )
            ax.plot(
                tpr,
                i,
        ...
        for i, (m, fpr_d) in enumerate(zip(ordered, fprs_disp)):
            c = _c(m)
            ax.plot(
                [0, fpr_d], [i, i], color=c, lw=0.9, alpha=0.38, solid_capstyle="round"
            )
            ax.plot(
                fpr_d,
                i,
        ...
        ax.axvline(
            alpha, color="0.25", ls="--", lw=0.9, alpha=0.8, label=f"α = {alpha}"
        )
    ...
    fig.suptitle(
        "Drift Detection: True and False Positive Rates", fontsize=10, fontweight="bold"
    )
...
def plot_delay_distributions(results: dict, save_dir: Path) -> plt.Figure:
    ...
    methods whose distribution rests on very few events.
    ...
    for ax, scen in zip(axes, scenarios):
        ...
        if violin_data:
            ...
            widths = []
            for nd in violin_ndets:
                w = max(0.7, np.sqrt(nd / max_n)) * 0.75
                if nd <= 2:
                    w = max(w, 0.62)
                elif nd <= 5:
                    w = max(w, 0.56)
            ...
            for body, col, nd, pos, data in zip(
                parts["bodies"], violin_cols, violin_ndets, violin_pos, violin_data
            ):
                ...
                span = x_max - x_min
                x_right_violin = max(x_right_violin, x_max)

                # Emphasize very short/small violins (tight delay distributions)
                if span <= 50.0:
                    body.set_alpha(0.60)
        ...
        for i in no_det_indices:
            m = ordered[i]
            ax.vlines(0, i - 0.32, i + 0.32, lw=1.5, colors=_c(m), alpha=0.80, zorder=6)

        # ── X-axis limits from actual rendered violin extent ──────────────────
        x_right_data = x_right_violin
        if x_right_data == 0.0:
            all_flat = np.concatenate([d for d in method_delays.values() if len(d) > 0])
            x_right_data = (
                float(np.percentile(all_flat, 99.5)) if len(all_flat) > 0 else 100.0
            )
        ...
        ax.set_yticks(y)
        ax.set_yticklabels(ordered, fontsize=tick_label_size)
...
def plot_cp_error_distribution(results: dict, save_dir: Path) -> Optional[plt.Figure]:
    ...
    n = len(plot_data)
    fig, axes = plt.subplots(
        1, n, figsize=(3.8 * n, 3.5), gridspec_kw={"wspace": 0.45}, squeeze=False
    )
    ...
    for ax, data in zip(axes, plot_data):
        errs = data["errors"]
        lo = max(0, int(np.floor(np.percentile(errs, 0.5))))
        hi = int(np.ceil(np.percentile(errs, 99.5)))
        rng = max(1, hi - lo)
        bw = max(1, rng // 60)  # ~60 bins regardless of range
        bins = np.arange(lo - bw / 2, hi + bw, bw)
        pad = rng * 0.04
    ...
    fig.suptitle(
        "PITMonitor Changepoint Estimation Error", fontsize=10, fontweight="bold"
    )
...
def plot_single_run_panels(
    ...
    fig.suptitle(
        f"PITMonitor Single Run — {scen_label}", fontsize=10, fontweight="bold"
    )
    ...
    leg = [
        ...
        Line2D(
            [0], [0], marker="o", ls="", color=C_POST, ms=3.5, label="Actual (post)"
        ),
    ...
    ax.set(xlabel="Sample", ylabel="PIT value", title="(b) PIT stream")
    ax.set_ylim(0, 1)
    ...
    leg_c.append(
        Line2D(
            [0], [0], color=C_SHIFT, ls="--", lw=1.4, label=f"True shift (t={shift})"
        )
    )
    ...
    if cp_est is not None:
        ...
        leg_c.append(
            Line2D([0], [0], color=C_CP, ls=":", lw=2.0, label=f"CP est. (t={cp_int})")
        )
    ...
    ax.set(
        xlabel="PIT value", ylabel="Density", title="(d) PIT distributions", xlim=(0, 1)
    )
...
def generate_latex_table(results: dict, save_dir: Path) -> str:
    ...
    for scen in scenarios:
        tprs = [
            results["results"][scen].get(m, {}).get("tpr", 0.0)
            for m in ALL_DETECTOR_NAMES
        ]
...
def generate_experiment_macros(results: dict, save_dir: Path) -> str:
    ...
    lines = [
        ...
        rf"\newcommand{{\expLEAOffA}}{{{cfg['n_post'] // 3:,}}}".replace(",", "{,}"),
        rf"\newcommand{{\expLEAOffB}}{{{2 * cfg['n_post'] // 3:,}}}".replace(
            ",", "{,}"
        ),
        "",
