...
\begin{abstract}
\noindent Deployed machine learning models often experience calibration drift as the world changes. To address this challenge, we present PITMonitor, a sequential method for detecting calibration changes in probabilistic models. Unlike many traditional calibration tests, PITMonitor provides \emph{anytime-valid} false alarm control: the probability of ever raising a spurious alarm is bounded by $\alpha$ for all time, without requiring a fixed horizon or stopping rule. A Bayesian changepoint estimator, absent from all compared baselines, identifies how far back model outputs were already corrupted. We prove Type I error control via Ville's inequality and evaluate PITMonitor on three drift scenarios from \texttt{river}'s FriedmanDrift dataset, comparing against the seven included stream-drift detectors. PITMonitor achieves second-highest TPR among all eight detectors on the two global drift scenarios while maintaining the only formally proven stream-level FPR guarantee; it fails to detect the local expanding drift scenario, a structural limitation we characterize theoretically. Code is available at \url{https://github.com/tristan-farran/pitmon}.
\end{abstract}
...
Probabilistic models deployed in production face a fundamental challenge: the world changes. Across many domains from medicine to finance, models may encounter non-stationary processes, regime shifts, and concept drift. When these shifts occur, model calibration can degrade drastically, leading to consequential issues downstream. In practice, calibration is often monitored using ad-hoc procedures such as periodic recalibration schedules, rolling-window hypothesis tests, threshold-based alerts on summary metrics, or manual inspection of residuals.
...
\begin{enumerate}
    ...
    \item \textbf{Practical efficiency:} the algorithm runs in $O(t \log t)$ time and $O(t)$ space for $t$ observations, with an $O(1)$ per-step recursive update.
\end{enumerate}
...
\end{equation}
Since the PITs arise from a continuous predictive CDF, ties occur with probability zero
($R_t - 1 = \#\{s < t : U_s < U_t\}$ almost surely), so this simplification is exact.
For discrete models with genuine ties, the formula generalises straightforwardly to
$p_t = \bigl(\#\{s \leq t : U_s < U_t\} + V_t \cdot \#\{s \leq t : U_s = U_t\}\bigr)/t$,
which the implementation uses directly.
...
\begin{equation}
    ...
    \leq M_{t-1}^{(\tau)}
\end{equation}
Thus each $(M_t^{(\tau)})_{t \geq \tau}$ is a non-negative supermartingale under $H_0$.  Since the changepoint $\tau$ is unknown, PITMonitor maintains a \emph{mixture} over all candidate starting times (Section~\ref{sec:method_mixture}), which inherits the supermartingale property by linearity of expectation.
...
Under exchangeability, p-values scatter uniformly and the learned histogram spreads mass roughly evenly across bins, yielding $\E[e] \approx 1$. If exchangeability breaks, p-values cluster in certain bins; the histogram learns these concentration patterns and achieves $\E[e] > 1$, generating detection power. We update the histogram \emph{after} computing $e_t$, ensuring $\hat{f}$ is $\mathcal{F}_{t-1}$-measurable (predictable), as required for the supermartingale property of the e-process.
...
\begin{proof}
By Proposition~\ref{prop:density}, any density $\hat{f}$ satisfying $\int_0^1 \hat{f}(p)\,dp = 1$ yields $e = \hat{f}(p)$ with $\E[e] = 1$ when $p \sim \Unif(0,1)$.  PITMonitor's histogram density $\hat{f}_{t-1}$ is constructed from $p_1, \ldots, p_{t-1}$ and is therefore $\mathcal{F}_{t-1}$-measurable (predictable).  Under $H_0$, Section~\ref{sec:pvals} establishes $p_t \mid \mathcal{F}_{t-1} \sim \Unif(0,1)$; thus
\begin{equation}
    \E[e_t \mid \mathcal{F}_{t-1}] = \int_0^1 \hat{f}_{t-1}(p)\,dp = 1.
\end{equation}
...
\begin{itemize}
    \item \textbf{PITMonitor}: $\alpha = \expAlpha$, $B = \expNbins$.  A key practical advantage is that PITMonitor has only two interpretable parameters: $\alpha$ directly and provably controls the stream-level false alarm probability (Theorem~\ref{thm:type1}), and $B$ controls the histogram resolution.  No pre-specified monitoring horizon, held-out null data, or calibration step is required.
    \item \textbf{Continuous-input baselines (ADWIN, KSWIN, PageHinkley)}: library-default parameters.  Their internal sensitivity parameters (ADWIN's $\delta$, KSWIN's significance level, PageHinkley's threshold) have indirect, horizon-dependent relationships to stream-level FPR.
    \item \textbf{Binary-input baselines (DDM, EDDM, HDDM\_A, HDDM\_W)}: library-default parameters.  These detectors require a binary error signal; we binarize via $b_t = \mathbf{1}[|r_t| > \theta]$ where $\theta$ is the \emph{median} of $|r_t|$ on the training data (giving $\approx 50\%$ pre-drift error rate).  The median is the most neutral, assumption-free threshold choice: it makes no assumption about the detectors' preferred operating error rate.  That binary detectors were designed for lower error rates ($\sim 5$--$20\%$) is an inherent limitation of applying them to regression monitoring, and one that PITMonitor avoids by consuming PIT values directly.
\end{itemize}
...
visualizes TPR and FPR per method and scenario, Figure~\ref{fig:single_run}
shows a representative single-run monitoring trace for the GRA scenario, and
Figure~\ref{fig:delay_dists} shows the distribution of detection delays.
...
\caption{TPR and FPR across all detectors and drift scenarios. The dashed red line marks $\alpha = \expAlpha$. River baselines are run at library defaults; PITMonitor is the only method with a proven stream-level FPR guarantee (Theorem~\ref{thm:type1}).}
...
\caption{Single-run PITMonitor trace on the GRA (abrupt) scenario. (a) Predicted vs.\ actual values with alarm marker. (b) PIT stream with 50-sample rolling mean. (c) Mixture e-process on a log scale; the dashed red line is the alarm threshold $1/\alpha$. (d) Pre- and post-shift PIT histograms illustrating the calibration change detected by the algorithm.}
\label{fig:single_run}
\end{figure}


\paragraph{Type I error control.}
PITMonitor achieves FPR of \resPMGRAFPR{} across all scenarios, consistent with the nominal $\alpha = \expAlpha$ guarantee proved in Theorem~\ref{thm:type1}.  This is the only detector in the comparison for which FPR control is formally guaranteed regardless of monitoring horizon.  ADWIN maintains \resADGRAFPR{} empirical FPR on the \expNstable-sample null window; however, this figure is specific to that window length.  Because ADWIN evaluates its Hoeffding bound at every observation, false alarm opportunities accumulate with the monitoring horizon: a practitioner monitoring over 25{,}000 samples at the same $\delta$ setting could expect substantially more spurious alarms than observed here.  PITMonitor's $\Prob(\text{ever alarm} \mid H_0) \leq \alpha$ bound holds regardless of how long monitoring continues.  Among the remaining baselines, DDM and HDDM\_A exhibit FPR of \resDDMGRAFPR{} and \resHDDMAGRAFPR{} respectively --- both above the nominal $\alpha$ and without a formal guarantee; KSWIN, PageHinkley, EDDM, and HDDM\_W exhibit FPR exceeding $89\%$ --- these detectors alarm on nearly every null run, rendering them unusable for practical monitoring without re-tuning.  This illustrates the fundamental challenge of deploying library-default baselines: parameters tuned for one setting (e.g., very short streams or very large drifts) can fail catastrophically in others.

\paragraph{Global drift (GRA, GSG).}
PITMonitor achieves \resPMGRATPR{} TPR with \resPMGRAFPR{} FPR on both global drift scenarios, with detection delay of \resPMGRADelay{} samples on GRA and \resPMGSGDelay{} on GSG.  This is the second-highest TPR among all eight detectors on both global scenarios.  Crucially, it is also the highest TPR among the two detectors that maintain FPR at or below the nominal $\alpha$: only PITMonitor and ADWIN satisfy this criterion on global drift, and PITMonitor's \resPMGRATPR{} TPR represents the best achievable sensitivity under a formally valid false alarm constraint.  ADWIN achieves \resADGRATPR{} TPR with \resADGRAFPR{} FPR and a mean delay of \resADGRADelay{} samples on both GRA and GSG; the identical delay across these scenarios reflects that ADWIN's Hoeffding bound is sensitive enough to detect even the early portion of the gradual transition window.  Among the binary baselines, DDM and HDDM\_A achieve \resDDMGRATPR{} and \resHDDMAGRATPR{} TPR respectively; DDM's mean delay of \resDDMGRADelay{} samples on GRA reflects slower adaptation on the binarized error stream.  The remaining detectors (KSWIN, PageHinkley, EDDM, HDDM\_W) have near-zero TPR as a direct consequence of their high FPR: having already fired during the null window, they are never credited with a true-positive detection.

\paragraph{Local expanding drift (LEA).}
PITMonitor achieves \resPMLEATPR{} TPR on the LEA scenario, correctly maintaining FPR control but failing to detect the drift. This reveals a meaningful limitation: LEA drift begins on a subset of the five relevant features, producing a small initial perturbation to the Gaussian predictive distribution that the histogram e-value does not accumulate evidence for quickly enough within the \expNpost-sample post-drift window. ADWIN's squared-residual statistic is more sensitive to partial distributional shifts than PITMonitor's PIT-based approach.

\paragraph{Detection delays.}
Figure~\ref{fig:delay_dists} shows the full distribution of detection delays across true-positive trials. PITMonitor's delay distribution is unimodal and moderately concentrated. Its mean delay of \resPMGRADelay{} samples on GRA reflects the amount of evidence the mixture e-process must accumulate before crossing $1/\alpha$; this is the explicit cost of the anytime-valid guarantee --- stricter evidentiary requirements mean the process is more conservative by design, not a failure of sensitivity. ADWIN's distribution reflects the Hoeffding bound of its adaptive window; its mean delay is identical on GRA and GSG because its sensitivity at library defaults is sufficient to detect even the earliest portion of the gradual transition window. HDDM\_A achieves shorter delay than PITMonitor on GRA (\resHDDMAGRADelay{} vs \resPMGRADelay{} samples) but lacks any formal FPR guarantee.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../experiment/out/fig_delay_distributions.png}
\caption{Detection delay distributions (true-positive trials only) across all detectors and drift scenarios. Methods with zero true-positive detections are marked with $\times$ at the bottom. Violins show the full delay distribution; the white bar marks the median.}
...
\paragraph{Scope.}
PITMonitor is designed for the continuous monitoring of deployed probabilistic models
where (a) false alarms have real costs --- unnecessary retraining, alert fatigue, and
loss of trust; (b) the monitoring horizon is indefinite or stopping is data-dependent;
(c) \emph{calibration} drift --- changes in predictive uncertainty, not merely raw
accuracy --- is the primary concern; and (d) accurate changepoint localisation is
operationally valuable.  For one-time calibration assessment (``is this model
calibrated?''), standard tools such as reliability diagrams or Expected Calibration
Error suffice.  PITMonitor addresses the harder, ongoing problem of monitoring with
explicit statistical guarantees.

\paragraph{Limitations.}

\textit{Local and partial drift.}
PITMonitor achieves zero TPR on the LEA scenario, and this failure warrants more than a surface acknowledgement --- it points to a genuine structural limitation.

The LEA configuration places the second and third expansion events far beyond the monitoring window, so at no point during the 2500-sample post-drift period has more than \emph{one} of Friedman's five active feature terms changed.  Under a well-calibrated model, the PIT perturbation scales with the shift in $\mathbb{E}[Y \mid X]$ relative to the predictive standard deviation.  Writing the post-drift PIT as $U_t \approx \Phi(Z_t + \delta_t)$ with $Z_t \sim \mathcal{N}(0,1)$ and $\delta_t = \Delta\mathbb{E}[Y \mid X_t]/\hat{\sigma}_t$, a single-feature drift produces a bias $\delta_t$ that is substantially smaller in magnitude than under GRA or GSG, where all five terms shift simultaneously.  The conformal p-values generated from these PITs are therefore only mildly non-uniform --- the departure from $\mathrm{Uniform}(0,1)$ is small, not zero --- and the histogram density estimator adjusts correspondingly slowly.

The mixture e-process accumulates evidence at a rate determined by how non-uniform the p-value stream actually is.  Each step contributes a multiplicative factor $E_t = \hat{f}(p_t)$ whose expectation under the post-drift distribution is $1 + \epsilon$ for some small $\epsilon > 0$ proportional to the KL divergence between the p-value distribution and uniform.  With a weak single-feature perturbation, $\epsilon$ is small, meaning the e-process must run for many steps before its product meaningfully exceeds 1.  The 2500-sample post-drift window provides a hard ceiling: if the total available multiplicative growth over those samples is insufficient to cross $1/\alpha$, no alarm fires regardless of how $\alpha$ is set.

That the failure persists even at $\alpha = 0.30$ --- where the required threshold is only $1/0.30 \approx 3.3$ --- is diagnostic.  It reveals that the mixture e-process grows so slowly during 2500 post-drift samples that even a six-fold reduction in the required threshold does not help.  This is not a threshold calibration issue; it is a power issue rooted in insufficient KL divergence from a single drifting feature within the observation budget.  Detecting single-feature LEA drift with PITMonitor would require substantially more post-drift data than the 2500-sample window, a more powerful density estimator optimised for small departures from uniformity, or a complementary feature-level monitor that isolates the drifting direction before feeding signals to the e-process.

\textit{Detection delay vs.\ FPR control.}
PITMonitor's anytime-valid guarantee comes at the cost of later detection compared to methods without such guarantees. The mean delay on GRA reflects the amount of evidence the e-process must accumulate before crossing $1/\alpha$. The most direct way to reduce delay is to accept a larger $\alpha$: setting $\alpha = 0.10$ lowers the threshold to 10, requiring less accumulated evidence before an alarm fires. Users should calibrate $\alpha$ against their tolerance for false alarms over the full expected deployment horizon rather than treating it as a fixed convention.

\textit{Exchangeability assumption.}
PITMonitor tests exchangeability of PITs. Non-exchangeability can also arise from
temporal dependence: autocorrelated PITs can trigger alarms even when the calibration
distribution is unchanged. This occurs in time series models, models with lagged
features, or whenever predictions are not independent across time. Practitioners should
check for autocorrelation in the PIT sequence before interpreting alarms in such
settings.

\textit{Histogram resolution.}
The number of bins $B$ controls the bias-variance tradeoff in the density estimator.
Smaller $B$ is more stable but slower to adapt; larger $B$ adapts faster at the cost
of more variance in the estimated density.
...
\begin{itemize}
    ...
    PITMonitor alongside ADWIN on squared residuals: PITMonitor provides changepoint
    localisation and anytime-valid FPR control; ADWIN provides faster detection of
    partial shifts at the cost of a higher false alarm rate.
\end{itemize}

\paragraph{Structural comparison to \texttt{river} baselines.}
Two dimensions distinguish PITMonitor from the baselines beyond the numerical results.

\textit{Parameter simplicity and FPR guarantees.}  PITMonitor exposes two interpretable
parameters: $\alpha$ directly and provably controls stream-level FPR for any monitoring
horizon (Theorem~\ref{thm:type1}), and $B$ sets the histogram resolution.  In contrast,
the \texttt{river} baselines expose internal sensitivity parameters (ADWIN's $\delta$,
KSWIN's significance level, PageHinkley's threshold, and a binarization threshold for
binary detectors) whose relationship to stream-level FPR is indirect,
horizon-dependent, and formally unguaranteed.  Achieving a target stream-level FPR
with these methods requires advance knowledge of the monitoring window length or
empirical calibration --- exactly what PITMonitor obviates.

\textit{Calibration specificity.}  PITMonitor operates on the full PIT distribution,
which encodes every aspect of calibration: systematic over- or under-confidence, shifts
in predictive mean, and changes in predictive variance all manifest as non-stationarity
in the PIT stream.  The \texttt{river} baselines operate on squared residuals --- an
accuracy proxy that conflates calibration drift with prediction error.  A model that
becomes systematically overconfident --- predicting intervals that are too narrow ---
while retaining the same conditional mean would produce unchanged squared residuals
but strongly non-uniform PITs; PITMonitor is the only method in the comparison
designed to detect this class of calibration failure.

\textit{Benchmark context.}  FriedmanDrift is \texttt{river}'s own canonical synthetic
benchmark, and ADWIN's library defaults were likely validated on comparable data.
Its \resADGRAFPR{} empirical FPR on the \expNstable-sample null window is not a formal
guarantee: longer deployments accumulate additional Hoeffding comparisons, increasing
the stream-level false alarm probability beyond what this experiment demonstrates.
...
Experiments on three FriedmanDrift scenarios, with all baselines run at library defaults, demonstrate that PITMonitor achieves competitive detection performance on global drift (GRA and GSG) --- \resPMGRATPR{} TPR with \resPMGRAFPR{} FPR and a unique changepoint localization capability absent from all \texttt{river} baselines --- while providing the only formally proven stream-level FPR guarantee (Theorem~\ref{thm:type1}).  The results also surface an honest limitation: PITMonitor has zero detection power on local expanding drift (LEA), while ADWIN detects this scenario with \resADLEATPR{} TPR at \resADLEAFPR{} FPR.

Two structural advantages persist beyond this comparison.  First, PITMonitor's FPR guarantee is \emph{horizon-independent}: whereas ADWIN's empirical false alarm rate grows with the number of observations monitored (each new sample triggering an additional Hoeffding comparison), PITMonitor provably satisfies $\Prob(\text{ever alarm} \mid H_0) \leq \alpha$ regardless of deployment duration.  Second, PITMonitor targets \emph{calibration specifically}: by operating on probability integral transforms rather than squared residuals, it is sensitive to changes in predictive uncertainty --- systematic overconfidence, underconfidence, or variance shifts --- that leave accuracy metrics unchanged and would be missed entirely by residual-based detectors.

Practitioners should view PITMonitor as the right tool for global calibration monitoring with strict FPR control, and consider complementing it with residual-based detectors when feature-level drift is anticipated.
