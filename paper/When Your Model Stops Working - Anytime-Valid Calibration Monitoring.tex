...
ADWIN employs adaptive windowing with statistical change detection via Hoeffding bounds \citep{bifet2007learning}. Its parameter $\delta$ bounds the false alarm probability \emph{per comparison at each time step} (with a Bonferroni-style correction $\delta' = \delta/n$ for window length $n$), not the probability of ever raising a false alarm over an unbounded stream. Aligning $\delta$ with a desired stream-level FPR requires knowing the monitoring window length in advance and deriving or calibrating the relationship — neither of which is required for PITMonitor. Furthermore, ADWIN operates on generic performance statistics such as squared residuals and does not provide changepoint estimates, only partially addressing the problem of reliable, informative, long-term calibration monitoring.
