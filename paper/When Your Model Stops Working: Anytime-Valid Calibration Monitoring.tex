...
\begin{abstract}
\noindent Deployed probabilistic models often experience concept drift as the data distribution changes over time. We present PITMonitor, a sequential method for detecting departures from exchangeability in probability integral transforms (PITs), which is a useful proxy for calibration drift. Unlike traditional calibration tests that require fixed evaluation windows, PITMonitor provides \emph{anytime-valid} false alarm control: the probability of ever raising a spurious alarm is bounded by $\alpha$, regardless of when monitoring stops. Under i.i.d. stable behavior (including stable miscalibration), PITs remain exchangeable and no alarm fires. When the PIT generating mechanism changes, exchangeability breaks and evidence accumulates. This includes calibration changes, but can also include case-mix or dependence changes even when the reliability mapping is unchanged. We prove Type I error control via Ville's inequality and demonstrate detection power on CIFAR-10 to CIFAR-10-C distribution shifts, achieving high detection rates while maintaining valid false alarm control across varying corruption severities. Code is available at \url{https://github.com/tristan-farran/pitmon}.
...
Accordingly, the guarantee should be read precisely as follows: PITMonitor is guaranteed to stay silent under exchangeable PITs (including static miscalibration), but it may alarm for any non-exchangeability source, not only reliability-curve drift.
...
We run 1,000 trials per condition and report Wilson score 95\% confidence intervals.
...
\begin{table}[ht]
\centering
\caption{CIFAR Part 3 comparison (clean $\to$ CIFAR-10-C Gaussian noise) using 1{,}000 trials per severity.}
\label{tab:cifar_part3_compare}
\begin{tabular}{@{}ccccc@{}}
\toprule
Severity & Method & FPR & TPR & Median Delay \\
\midrule
...
5 & ADWIN & 0.0\% & 100.0\% & 116.0 \\
5 & KSWIN & 4.0\% & 47.3\% & 29.0 \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Part 3 Baseline Comparisons (Delivery Demo).}
We also include the Part 3 benchmark from the delivery demo notebook, where shift magnitude is controlled directly (H$_0$, 30\%, 60\%, 100\%).
Again, all methods are run on matched streams for 1{,}000 trials per condition.
Table~\ref{tab:delivery_part3_compare} reports the observed FPR/TPR/delay values.

\begin{table}[ht]
\centering
\caption{Delivery demo Part 3 comparison using 1{,}000 trials per shift level.}
\label{tab:delivery_part3_compare}
\begin{tabular}{@{}ccccc@{}}
\toprule
Shift & Method & FPR & TPR & Median Delay \\
\midrule
H$_0$ & PITMonitor & 3.6\% & 0.0\% & n/a \\
H$_0$ & DDM & 11.2\% & 3.2\% & 78.0 \\
H$_0$ & EDDM & 83.0\% & 7.6\% & 55.0 \\
H$_0$ & ADWIN & 0.0\% & 0.0\% & n/a \\
H$_0$ & KSWIN & 0.7\% & 2.1\% & 88.0 \\
\midrule
...
60\% & KSWIN & 0.7\% & 99.3\% & 21.0 \\
\midrule
100\% & PITMonitor & 2.9\% & 97.1\% & 30.0 \\
100\% & DDM & 11.6\% & 88.4\% & 20.0 \\
100\% & EDDM & 83.7\% & 16.3\% & 14.0 \\
100\% & ADWIN & 0.0\% & 100.0\% & 56.0 \\
100\% & KSWIN & 0.9\% & 99.1\% & 18.0 \\
\bottomrule
\end{tabular}
\end{table}
